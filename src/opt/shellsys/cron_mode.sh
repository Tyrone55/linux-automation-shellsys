#!/usr/bin/env bash
# cron_mode.sh — 切换定时任务模式（Rocky 10）
# 用法：sudo /opt/shellsys/cron_mode.sh demo|normal|status|run-now

set -Eeuo pipefail
trap 'echo "[FATAL] cron_mode.sh failed at line $LINENO" >&2; exit 4' ERR
...
status(){
  ...
  echo "=== 最近日志 ==="
  tail -n 3 "$LOG_DIR/cron_wrapper.log" 2>/dev/null || echo "(无 wrapper 日志)"
  tail -n 3 "$LOG_DIR/main_$(date +%F).log" 2>/dev/null || echo "(无 main 日志)"
}
